...
if __name__ == '__main__':
    ...
    driver.set_window_position(960 + int((args.index_number % 4) / 2) * 480, ((args.index_number % 4) % 2) * 450)
    driver.get(UPWORK_URL)
    
    # ele = driver.find_element(By.TAG_NAME, 'html')
    # actions = ActionChains(driver)
    # actions.click(on_element = ele)
    # actions.key_down(Keys.CONTROL).send_keys(Keys.SUBTRACT).send_keys(Keys.SUBTRACT).send_keys(Keys.SUBTRACT).send_keys(Keys.SUBTRACT).send_keys(Keys.SUBTRACT).send_keys(Keys.SUBTRACT).send_keys(Keys.SUBTRACT).key_up(Keys.CONTROL)
    # actions.perform()
    ...
    while True:
        ...
        match newURL.split('/')[-1]:            
            ...
            case _:
                time.sleep(1)
                driver.get('https://www.upwork.com/nx/create-profile/')
                continue
        oldURL = newURL
    try:
        WebDriverWait(driver, 30).until(
            EC.visibility_of_element_located((By.CSS_SELECTOR, 'div.up-modal-footer button'))
        )
    except:
        pass
