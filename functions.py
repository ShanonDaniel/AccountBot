...
def submit(driver)->bool:
    ...
    try:
        WebDriverWait(driver, 10).until(
            EC.element_to_be_clickable((By.CSS_SELECTOR, 'button[data-qa="active-editor-edit-btn"]'))
        )
    except:
        print('Can\'t load edit buttons!')
        pass

    time.sleep(1)
    try:
        eles = driver.find_elements(By.CSS_SELECTOR, 'button[data-qa="active-editor-edit-btn"]')
        for ele in eles:
            try:
                time.sleep(1)
                ...
                try:
                    WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located((By.CSS_SELECTOR, 'button[data-qa="btn-save"]]'))
                    )
                except:
                    print('Can\'t load save button!')
                    pass
                try:
                    ele = driver.find_element(By.CSS_SELECTOR, 'button[data-qa="btn-save"]')
                    ele.click()
                ...
                print('Can\'t click all edit and save buttons!')
                pass
    except:
        print('Can\'t click edit buttons!')
        pass
